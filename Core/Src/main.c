...
int main(void)
{
  ...
  MX_I2C1_Init();
  /* USER CODE BEGIN 2 */
  if(HAL_I2C_IsDeviceReady(&hi2c1, MODULE_ADDR, 2, 1000) == HAL_OK) {
  ...
  while (1)
  {
	  ...
	  snprintf((char *) result, sizeof(result), "%d+%d=%d", n1,n2,n1+n2);
	  lcd_send_str((char *) result);
	  HAL_Delay(2000);
	  lcd_clear();
	  lcd_send_str("Exemplo lcd");
	  HAL_Delay(2000);
	  lcd_clear();
